...
3. **IARM Communication**: Implementation serializes parameters and calls ctrlm via IARM bus. The payload is forwarded unchanged, including the `ptt_listen` type and optional `macAddr`.
4. **Controller Selection**: For `ptt_listen`, ctrlm targets the requested paired BLE RCU or requires exactly one connected paired RCU when `macAddr` is omitted.
